    ...
    public function __construct($componentArgs = array())
    {
        ...
        if (get_showtime('include_room_booking_functionality') && $mrConfig['showAvailabilityCalendar'] == '1') {
            if ($mrConfig[ 'singleRoomProperty' ]) {
                $calendar = $MiniComponents->specificEvent('06000', 'srp_calendar', array('output_now' => false, 'property_uid' => $property_uid, 'months_to_show' => 6, 'show_just_month' => false));/*OPE 24->6*/
            } elseif ($mrConfig[ 'showRoomsInPropertyDetails' ]) {
                $calendar = $MiniComponents->specificEvent('06000', 'mrp_calendar', array('output_now' => false, 'property_uid' => $property_uid, 'months_to_show' => 6, 'show_just_month' => false));/*OPE 24->6*/
            }
        }
