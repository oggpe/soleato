        ...
        if (isset($property_list_layouts[ $layout ][ 'custom_task' ])) {
            ...
            if (!empty($propertysToShow)) {
                ...
                foreach ($propertysToShow as $propertys_uid) {
                    ...
                    if ($mrConfig[ 'is_real_estate_listing' ] == 0 && !$plugin_will_provide_lowest_price && $jomres_property_list_prices->lowest_prices[$propertys_uid]['PRICE'] != jr_gettext('_JOMRES_PRICE_ON_APPLICATION', '_JOMRES_PRICE_ON_APPLICATION', '', true, false) /*OPE*/ && ! $jomres_property_list_prices->lowest_prices[$propertys_uid]['SHOW_MIN_TARIFF']) {/*&& $stayDays > 1)*/
                    ...
                    if (strlen($current_property_details->multi_query_result[ $propertys_uid ][ 'property_name' ]) > 24 && $layout == 'tile') {
                        $property_deets[ 'PROP_NAME' ] = jr_substr($current_property_details->multi_query_result[ $propertys_uid ][ 'property_name' ], 0, 20).'&hellip;';
                    } else {
