...
function get_property_module_data($property_uid_array, $alt_template_path = '', $alt_template_name = '', $vertical = false)
{
	...
	/*OGP START*/
    $MiniComponents->triggerEvent('01012', array('property_uids' => $property_uid_array));
    /*OGP END*/
    ...
    foreach ($property_uid_array as $property_uid) {
        if ($property_uid > 0) {
            ...
            /*OGP START*/
            $MiniComponents->triggerEvent('01012', array('property_uid' => $property_uid));
            $mcOutput = $MiniComponents->getAllEventPointsData('01012');
            if (!empty($mcOutput)) {
                foreach ($mcOutput as $key => $val) {
                    if (!is_null($val)) {
                        $result = array_merge($property_data, $val);
