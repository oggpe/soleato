    ...
    public function gather_lowest_prices_multi($property_uids = array(), $lowest_ever = false, $hide_rpn = false)
    {
        ...
        if (!empty($property_uids)) {
            ...
            if (!empty($property_uids_to_query)) {
                ...
                /*OPE*/
                /*query used to get the min tariff for each property, used to replace the _JOMRES_PRICE_ON_APPLICATION label (POA) */
                $tmpquery = 'SELECT property_uid,  min(roomrateperday) as roomrateperday FROM #__jomres_rates WHERE property_uid IN ('.jomres_implode($property_uids).") AND roomrateperday > 0 group by property_uid";
                $tariffListNoDate = doSelectSql($tmpquery);
                ...
                foreach ($property_uids_to_query as $property_uid) {
                    ...
                    $grand_total = 0.00;
                    /*OPE added start*/
                    /*$show_min_tariff_flag is used when no tariff is available for the selected dates
                    in this case $show_min_tariff_flag is used when no tariff is available for the selected dates*/
                    $show_min_tariff_flag = false;
                    $pricesFromArrayNoDates = array();
                    if (!empty($tariffListNoDate)) {
                        foreach ($tariffListNoDate as $t) {
                            $pricesFromArrayNoDates[$t->property_uid] = $t->roomrateperday;
                        }
                    }
                    /*OPE added end*/
                    ...
                    if (!empty($tariffList)) {
                        ...
                        if ($mrConfig[ 'is_real_estate_listing' ] == 0) {
                            if (isset($pricesFromArray[ $property_uid ])) {
                                ...
                                */
                                /*OPE added start*/
                                $show_min_tariff_flag = true;
                                $price = output_price($pricesFromArrayNoDates[$property_uid], '', true, true);
                                $pre_text = jr_gettext('_JOMRES_TARIFFSFROM', '_JOMRES_TARIFFSFROM', false, false);
                                if ($mrConfig[ 'perPersonPerNight' ] == '0') {
                                    $post_text = '&nbsp;'.jr_gettext('_JOMRES_FRONT_TARIFFS_PN', '_JOMRES_FRONT_TARIFFS_PN');
                                } else {
                    ...
                    $this->lowest_prices[$property_uid] = array('PRE_TEXT' => $pre_text, 'PRICE' => $price, 'POST_TEXT' => $post_text, 'RAW_PRICE' => $raw_price, 'PRICE_NOCONVERSION' => $price_no_conversion, 'PRICE_CUMULATIVE' => $grand_total, 'SHOW_MIN_TARIFF' => $show_min_tariff_flag); /*OPE $show_min_tariff_flag*/
                }
            }
